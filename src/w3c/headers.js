...
hb.registerHelper("showPeople", function(name, items = []) {
  ...
  let ret = "";
  const bns = new Array(items.length).fill().map((_, i) => `_:editor${i + 1}`)
  for (let i = 0, n = items.length; i < n; i++) {
    const bn = (self.doRDFa && name === "Editor") ? (i + 1 < n ? `_:editor${i + 1}` : "rdf:nil") : null;
    ret += getItem(items[i], bn, re).outerHTML;
  }
  return new hb.SafeString(ret);

  function getBn(p, i) {
    return (self.doRDFa && name === "Editor") ? (i + 1 < n ? `_:editor${i + 1}` : "rdf:nil") : null;
  }
  function getItem(p, i) {
    ...
    if (self.doRDFa && name === "Editor") {
      dd.appendChild(html`\n<span property='rdf:rest' resource='${bn}'></span>\n`);
    }
    return dd;
  }
